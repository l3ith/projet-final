...
let musicToPlay;
let g_theme = localStorage.getItem("theme");
let user = localStorage.getItem("auth");

window.onload = ()=>{
  if(!user){
    window.location.href = "/projet%20final%20v2/inscription.html";
  }
  document.getElementById("u_name").innerText = JSON.parse(user).email;
  if(g_theme){
  if(g_theme === "Light"){
...
if (url.get("type")) {
  ...
  <div style="width:200px;">
    ...
    overflow: hidden;">${song.title} <br> <span style="color: #9d9aa1;font-size: 14px;">${song.artist.name}</span> </h1>
  </div>
  </div>
  ...
  <div class="controlHolder" style="font-size: 14px;width:100px"> <span style="color:#9d9aa1 ;">${Math.floor(
    song.duration / 60
